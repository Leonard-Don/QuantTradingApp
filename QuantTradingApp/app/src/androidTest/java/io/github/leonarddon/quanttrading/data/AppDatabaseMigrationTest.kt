...
class AppDatabaseMigrationTest {
    ...
    fun migrationFromEmptyVersion1CreatesCurrentTables() {
        ...
        try {
            ...
            assertTrue("price_alerts should exist", "price_alerts" in tables)
            val userColumns = database.openHelper.readableDatabase.columnNames("user_state")
            assertTrue("accountStatus should exist", "accountStatus" in userColumns)
            assertTrue("legacy extra columns should be removed", "legacyFlag" !in userColumns)
            assertTrue("legacy extra columns should be removed", "legacyExpiresAt" !in userColumns)
            val priceAlertColumns = database.openHelper.readableDatabase.columnNames("price_alerts")
            assertTrue("targetPrice should exist", "targetPrice" in priceAlertColumns)
            assertTrue("direction should exist", "direction" in priceAlertColumns)
            assertTrue("lastTriggeredAt should exist", "lastTriggeredAt" in priceAlertColumns)
    ...
    @Test
    fun migrationFromLegacyVersion7PreservesLocalStateAndDropsLegacyAccessColumns() {
        createLegacyDatabase(version = 7).use { database ->
            database.execSQL(
                "CREATE TABLE `user_state` (" +
                    "`id` TEXT NOT NULL, `displayName` TEXT NOT NULL, `isLoggedIn` INTEGER NOT NULL, " +
                    "`legacyFlag` INTEGER NOT NULL, `legacyExpiresAt` INTEGER NOT NULL, " +
                    "`createdAt` INTEGER NOT NULL, `lastLoginAt` INTEGER NOT NULL, PRIMARY KEY(`id`))"
            )
            database.execSQL(
                "INSERT INTO `user_state` (`id`, `displayName`, `isLoggedIn`, `legacyFlag`, " +
                    "`legacyExpiresAt`, `createdAt`, `lastLoginAt`) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ...
        try {
            ...
            database.openHelper.readableDatabase.query(
                "SELECT displayName, isLoggedIn, accountStatus FROM user_state WHERE id = ?",
                arrayOf(LOCAL_USER_ID)
            ).use { cursor ->
                assertTrue(cursor.moveToFirst())
                assertEquals("本机测试用户", cursor.getString(0))
                assertEquals(1, cursor.getInt(1))
                assertEquals("本机账号数据仅保存在设备上", cursor.getString(2))
            }
        } finally {
            database.close()
        }
    ...
    private companion object {
        const val TEST_DB = "quanttrading_quant_migration_test.db"
    }
}
