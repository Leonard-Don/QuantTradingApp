...
class CommunityDigestPolicyTest {
    ...
    @Test
    fun featuredPostAddsSamplePoolResearchAction() {
        val report = CommunityDigestPolicy.build(
            post(category = "专题研究", content = "样本池模板只用于复盘训练。"),
            emptyList()
        )

        assertTrue(report.keyPoints.any { it.contains("样本池") })
        assertTrue(report.riskNotes.any { it.contains("研究参考口径") })
        assertTrue(report.researchActions.any { it.contains("自选池体检") })
    }

    private fun post(category: String, content: String): Post {
        return Post(
            id = "1",
            author = "测试作者",
            avatar = "",
