...
import io.github.leonarddon.quanttrading.model.StockFilterPolicy
import io.github.leonarddon.quanttrading.model.StockInfo
import io.github.leonarddon.quanttrading.model.StockSearchIndex
import io.github.leonarddon.quanttrading.model.hasMovingAverageData
    ...
    private fun applyCurrentFilter() {
        ...
        _dataStatus.value = when {
            ...
            result.sourceRequired -> {
                "$effectiveSortMode 需要接入专门合规数据源，当前不展示模拟名单。"
            }
            effectiveSortMode == "多因子" -> {
                "$baseDataStatus\n当前条件：PE 1-35、PB 0.1-6，并按涨跌幅和估值排序；仅基于当前行情池可用字段。"
            }
            effectiveSortMode == "每日样本池" -> {
