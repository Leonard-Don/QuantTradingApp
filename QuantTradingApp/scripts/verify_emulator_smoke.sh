...
MAIN_ACTIVITY="io.github.leonarddon.quanttrading/.MainActivity"
...
swipe_review_tabs_left() {
  local center
  local y=240
  if center="$(node_center "$REVIEW_XML" "resource-id" "io.github.leonarddon.quanttrading:id/tabLayout")"; then
    y="${center##* }"
  fi
...
swipe_stock_filters_left() {
  local center
  local y=500
  if center="$(node_center "$VIP_CHIP_XML" "resource-id" "io.github.leonarddon.quanttrading:id/filterChips")"; then
    y="${center##* }"
  fi
...
dump_ui "$MAIN_XML"
assert_focus_contains "io.github.leonarddon.quanttrading/.MainActivity"
...
dump_ui "$ACCOUNT_XML"
assert_focus_contains "io.github.leonarddon.quanttrading/.ui.vip.VipActivity"
...
for _ in 1 2 3 4 5; do
  if has_node "$STOCK_XML" "resource-id" "io.github.leonarddon.quanttrading:id/tvStockName"; then
    break
  fi
  sleep 1
  dump_ui "$STOCK_XML"
done
assert_node "$STOCK_XML" "resource-id" "io.github.leonarddon.quanttrading:id/tvStockName"
...
tap_node "$STOCK_XML" "resource-id" "io.github.leonarddon.quanttrading:id/tvStockName"
sleep 1
...
dump_ui "$AUTH_XML"
assert_focus_contains "io.github.leonarddon.quanttrading/.ui.auth.AuthActivity"
