...
``QUANTTRADING_*`` environment variables:
...
BACKEND_ONLY_EXAMPLE_VARS = frozenset(
    {
        "QUANTTRADING_DB_PATH",
        "QUANTTRADING_PAYMENT_CALLBACK_SECRET",
        "QUANTTRADING_REQUIRE_CALLBACK_SIGNATURE",
        "QUANTTRADING_ADMIN_TOKEN",
    }
)
...
def _example_exports() -> set[str]:
    ...
    return set(re.findall(r"^export\s+(QUANTTRADING_[A-Z_]+)=", text, flags=re.MULTILINE))


def _script_references() -> set[str]:
    text = VERIFY_SCRIPT.read_text(encoding="utf-8")
    return set(re.findall(r"QUANTTRADING_[A-Z_]+", text))


def _workflow_env_keys() -> set[str]:
    text = RELEASE_WORKFLOW.read_text(encoding="utf-8")
    return set(re.findall(r"^\s+(QUANTTRADING_[A-Z_]+):\s*\$\{\{", text, flags=re.MULTILINE))
...
def test_release_env_example_declares_every_script_variable() -> None:
    ...
    assert not missing, (
        "verify_paid_release_config.sh references QUANTTRADING_ vars not declared in "
...
def test_release_env_example_has_no_unknown_android_vars() -> None:
    ...
    assert not extras, (
        f"release.env.example declares QUANTTRADING_ vars that are neither referenced "
...
def test_release_workflow_env_matches_script_references() -> None:
    ...
    assert not only_in_workflow, (
        "release-gate.yml exposes QUANTTRADING_ secrets that the verify script never "
    ...
    assert not only_in_script, (
        "verify_paid_release_config.sh reads QUANTTRADING_ vars that release-gate.yml "
...
def test_script_references_a_nonempty_set() -> None:
    ...
    assert _script_references(), (
        "no QUANTTRADING_* references parsed from verify_paid_release_config.sh - "
