...
object StockFilterPolicy {
    ...
    private val vipFilters = setOf("高级多因子(VIP)", "主力资金(VIP)", "龙虎榜(VIP)", "每日精选(VIP)")
    private val sourceRequiredFilters = setOf("主力资金(VIP)", "龙虎榜(VIP)")

    fun isVipFilter(sortMode: String): Boolean = sortMode in vipFilters
    ...
    fun filter(
        ...
        val vipBlocked = isVipFilter(selectedSortMode) && !hasStockVip
        val effectiveSortMode = if (vipBlocked) "全部" else selectedSortMode
        ...
        filtered = when (effectiveSortMode) {
            ...
            "高级多因子(VIP)" -> filtered.filter { it.pe in 1.0..35.0 && it.pb in 0.1..6.0 }
                .sortedWith(compareByDescending<StockInfo> { it.changePercent }.thenBy { it.pe })
            "主力资金(VIP)" -> emptyList()
            "龙虎榜(VIP)" -> emptyList()
            "每日精选(VIP)" -> filtered.sortedWith(
