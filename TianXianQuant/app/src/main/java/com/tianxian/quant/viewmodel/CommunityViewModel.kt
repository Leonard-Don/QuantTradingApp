...
class CommunityViewModel : ViewModel() {
    ...
    private val categories = listOf("全部", VIP_CATEGORY, "投资逻辑", "技术方法", "量化研究", "复盘笔记", "新手交流")
    ...
    fun addPost(title: String, content: String, category: String, isVip: Boolean) {
        viewModelScope.launch {
            LocalStateRepository.addPost(title, content, category, isVip)
            loadPosts()
        }
    }
    ...
    fun refreshVipState() {
        viewModelScope.launch {
            ...
            updateCommunityStats(_posts.value.orEmpty())
        }
    }
    ...
    private fun applyCategoryFilter() {
        ...
        val filtered = if (category == "全部") {
            allPosts
        } else if (category == VIP_CATEGORY) {
            allPosts.filter { it.isVip }
        } else {
    ...
    private fun updateCommunityStats(visiblePosts: List<Post>) {
        val category = _selectedCategory.value ?: "全部"
        val vipCount = allPosts.count { it.isVip }
        val lockedVipCount = if (_isVipActive.value == true) 0 else visiblePosts.count { it.isVip }
        _communityStats.value = CommunityStats(
            selectedCategory = category,
            totalCount = allPosts.size,
            visibleCount = visiblePosts.size,
            vipCount = vipCount,
            lockedVipCount = lockedVipCount
        )
    }
