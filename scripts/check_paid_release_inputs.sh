...
check_https_url TIANXIAN_PRODUCTION_API_BASE_URL "${TIANXIAN_PRODUCTION_API_BASE_URL:-}"
check_https_url TIANXIAN_PRIVACY_POLICY_URL      "${TIANXIAN_PRIVACY_POLICY_URL:-}"
check_https_url TIANXIAN_TERMS_URL               "${TIANXIAN_TERMS_URL:-}"
check_https_url TIANXIAN_DATA_DISCLAIMER_URL     "${TIANXIAN_DATA_DISCLAIMER_URL:-}"
check_email     TIANXIAN_SUPPORT_EMAIL           "${TIANXIAN_SUPPORT_EMAIL:-}"

if [[ -n "${TIANXIAN_RELEASE_KEYSTORE:-}" && ! -f "$TIANXIAN_RELEASE_KEYSTORE" ]]; then
  errors+=("TIANXIAN_RELEASE_KEYSTORE points to a missing file")
fi
